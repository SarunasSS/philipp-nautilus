...
The bars-with-fills chart reads bars from the engine cache after the backtest. Nautilus
retains 10,000 bars per type by default; a custom `--chart-bar-limit` changes that
capacity only when visualization is enabled. `--no-visualize` uses the engine default.
...
Backtests use hedging so separate Entry stratlets can run at the same time on one instrument.
The earlier netted HTF sweep + CISD runs skipped overlapping signals; their results are not
comparable with the hedged runs. Live Tradovate remains netted and keeps one active entry per strategy.
...
`--chart-bar-type` may be omitted because the first requested type is the execution stream. It is
spelled out above to make the chart selection explicit.
...
plain `NQ.c.0.GLBX-5-MINUTE-LAST-INTERNAL` can only be aggregated from prints, so it selects tick
data. `--visualize` and the `--chart-*` flags do nothing on the tick path, because
...
`--chart-bar-type` may be omitted because the first requested type is the strategy bar stream. It
is spelled out above to make the chart selection explicit.
...
below and is easy to mistake for a rules difference. `--chart-bar-type` is optional because the
requested strategy bar type is the chart default; it is explicit above for readability.
...
`--max-trades-per-day`; hedged backtest positions can overlap. Exits are `--take-profit-points` above and
...
`--chart-bar-type` may be left at its default for both plain and composite forms: the first requested
strategy bar is charted. Set it only to select another bar type cached during the run.
...
`--bar-type` is the only strategy option and is required. Repeat it to load more than one stream;
the first selected type configures the subscription strategy.
