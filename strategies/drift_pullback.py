...
def _stop_price(
    ...
    offset = (points_to_ticks(stop_points, tick_size) + slip_ticks) * tick_size
...
def _take_profit_price(
    ...
    tick_size = instrument.price_increment.as_double()
    offset = points_to_ticks(target_points, tick_size) * tick_size
    if direction == TradeDirection.LONG:
        return instrument.make_price(entry_price + offset)
...
class DriftPullbackStrategy(BaseStrategy):
    def __init__(self, config: DriftPullbackStrategyConfig) -> None:
        ...
        self._entry_start, self._entry_end = parse_time_range(
            config.entry_window, "entry_window", "10:30-15:30", "10:30",
        )
        self._vwap_start, self._vwap_end = parse_time_range(
            config.vwap_window, "vwap_window", "10:30-15:30", "10:30",
        )
        self._session_cutoff = parse_time(config.session_cutoff, "session_cutoff", "10:30")
    ...
    def _on_vwap_bar(self, bar: Bar) -> None:
        ...
        if not self._vwap.accumulated:
            ...
            first_close = seconds_since_midnight(self._vwap_start) + self._vwap_interval_seconds
            self._vwap.started_at_open = seconds_since_midnight(bar_time) <= first_close
        ...
        self._vwap.price_volume += typical_price(bar) * bar.volume.as_double()
    ...
    def _on_vwap_tick(self, tick: TradeTick) -> None:
        ...
        if not self._vwap.accumulated:
            ...
            first_print = seconds_since_midnight(self._vwap_start) + self._vwap_interval_seconds
            self._vwap.started_at_open = seconds_since_midnight(stamped_time) < first_print
            self._vwap.accumulated = True
            if not self._vwap.started_at_open:
                self.log.warning(
