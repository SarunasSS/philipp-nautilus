...
def _exit_price(instrument: Instrument, entry_price: float, points: float, above: bool) -> Price:
    ...
    offset = points_to_ticks(points, tick_size) * tick_size
    if above:
...
class VaultBreakStrategy(BaseStrategy):
    def __init__(self, config: VaultBreakStrategyConfig) -> None:
        ...
        self._earliest_entry = parse_time(config.earliest_entry, "earliest_entry", "11:00")
        self._flatten_time = parse_time(config.flatten_time, "flatten_time", "11:00")
    ...
    def _on_session_bar(self, bar: Bar) -> None:
        ...
        session.vwap_price_volume += typical_price(bar) * bar.volume.as_double()
        ...
        # 4. The entry. Live netting allows one open trade; hedged backtests can overlap. The counter
        #    moves on the decision, whether or not the order goes on to fill.
        if (
            ...
            entry = Entry(
                ...
                flatten_ns=boundary_ns(bar_close.date(), self._flatten_time, self._timezone),
            )
            self._entry = entry
        ...
        if session.bars_below_vwap >= self.config.vwap_exit_bars:
            active_entries = [
                stratlet
                for stratlet in self._stratlets
                if isinstance(stratlet, Entry) and not stratlet.completed and not stratlet.exit_requested
            ]
        else:
            active_entries = []
