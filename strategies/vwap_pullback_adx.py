...
from strategies.common import parse_time_range
...
from strategies.common import typical_price
...
class VwapPullbackAdxStrategy(BaseStrategy):
    def __init__(self, config: VwapPullbackAdxStrategyConfig) -> None:
        ...
        self._orb_start, self._orb_end = parse_time_range(
            config.opening_range_window, "opening_range_window", "09:30-10:00", "16:55",
        )
        self._vwap_start, self._vwap_end = parse_time_range(
            config.vwap_window, "vwap_window", "09:30-10:00", "16:55",
        )
        self._entry_start, self._entry_end = parse_time_range(
            config.entry_window, "entry_window", "09:30-10:00", "16:55",
        )
        self._eod_flat_time = parse_time(config.eod_flat_time, "eod_flat_time", "16:55")
    ...
    def _stop_price(self, level: float, direction: TradeDirection) -> Price:
        ...
        offset = points_to_ticks(self.config.stop_buffer_points, tick_size) * tick_size
    ...
    def _on_session_bar(self, bar: Bar) -> None:
        ...
        if in_vwap:
            if session.vwap_seen:
                session.vwap_price_volume += typical_price(bar) * volume
        ...
        # 11. The gate stack: live netting allows one open trade, while hedged backtests can overlap.
        #     A live signal arriving while a trade is still open is delayed, not spent. The
        ...
        #     minute later.
        if self.config.live and self._entry is not None and not self._entry.completed:
            return
        ...
        entry = Entry(
            ...
            flatten_ns=boundary_ns(bar_close.date(), self._eod_flat_time, self._timezone) if self.config.eod_flat else None,
        )
        self._entry = entry
